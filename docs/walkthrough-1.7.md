...
**Use this document if you want to run service-catalog on Kubernetes clusters version 1.7.0 and
above**
...
Service-catalog runs a server that is meant to be run behind an
[aggregated API server](https://kubernetes.io/docs/concepts/api-extension/apiserver-aggregation/).

Installation has a few steps to enable this.

#### Step 1: Generate TLS Credentials

The Kubernetes aggregation software requires that you provide it with a TLS certificate authority,
key and certificate to indicate how to communicate securely with the service-catalog API server. 
We have provided a [script](../charts/catalog/aggregation-tls-setup.sh) to automatically create all
these files. It requires the [`cfssl` tools](https://github.com/cloudflare/cfssl), so before you run
...
```console
./charts/catalog/aggregation-tls-setup.sh
```

#### Step 2: Install service-catalog

Now that the TLS files have been created, simply run the following Helm command to install 
service-catalog:
...
You may add the following flag to use Third Party Resource storage instead of etcd storage:

```console
--set apiserver.storage.tpr,apiserver.storate.tpr.globaNamespace=catalog
```

Regardless of whether you do this or not, the remainder of the steps in this
...
To proceed, we must download and install `kubectl` version 1.7 or newer. Do so by following the 
[standard instructions](https://kubernetes.io/docs/tasks/kubectl/install/).
...
We'll assume hereafter that all `kubectl` commands are using this newly-installed executable.
...
Finally, delete the `APIService` that was created by the service-catalog installation:
